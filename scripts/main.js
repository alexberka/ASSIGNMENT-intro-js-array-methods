...
// .forEach()
const renderCards = (array = referenceList) => {
  let refStuff = "";
...
const toggleCart = (event) => {
  if (event.target.id.includes("fav-btn")) {
   const [, cardId] = event.target.id.split("--")
   const changeIndex = referenceList.findIndex(item => item.id === Number(cardId))
   referenceList[changeIndex].inCart = !referenceList[changeIndex].inCart
   cartTotal()
   renderCards()
  }
}
